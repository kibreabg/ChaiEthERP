    ...
    public partial class frmVehicleApproval : POCBasePage, IVehicleApprovalView
    {
        ...
        private void SendEmailDriver(VehicleRequestStatus VRS)
        {
            foreach (VehicleRequestDetail assignedVehicle in _presenter.CurrentVehicleRequest.VehicleRequestDetails)
            {
                if (assignedVehicle.AppUser != null)
                {
                    string message = "You are assigned to give a drive to " + (_presenter.CurrentVehicleRequest.AppUser.FullName).ToUpper() 
                        + " with Vehicle Request Number of " + _presenter.CurrentVehicleRequest.RequestNo
                        + " and your assigned Car Plate Number is " + (assignedVehicle.PlateNo).ToUpper() 
                        + ". Please use Project ID of " + _presenter.CurrentVehicleRequest.Project.ProjectCode 
                        + ". Your Destination is " + _presenter.CurrentVehicleRequest.Destination 
                        + ". Your Departure Date is " + _presenter.CurrentVehicleRequest.DepartureDate.ToString() 
                        + " and Return Date is " + _presenter.CurrentVehicleRequest.ReturningDate.ToString();
